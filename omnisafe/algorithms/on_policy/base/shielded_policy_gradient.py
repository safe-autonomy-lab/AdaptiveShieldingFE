...
class ShieldedPolicyGradient(BaseAlgo):
    ...
    def _init_model(self) -> None:
        ...
        self._cfgs.shield_cfgs['env_info'] = self._cfgs.env_id.split('-')[0][:-1]
    ...
    def _init(self) -> None:
        ...
        """
        self._buf: VectorShieldedOnPolicyBuffer = VectorShieldedOnPolicyBuffer(
            obs_space=self._env.observation_space,
            act_space=self._env.action_space,
    ...
    def _init_log(self) -> None:
        ...
        +-----------------------+----------------------------------------------------------------------+
        | Safety/shield_violation | Average shield violation of the epoch.                            |
        +-----------------------+----------------------------------------------------------------------+
        | Time                  | Total time.                                                          |
        +-----------------------+----------------------------------------------------------------------+
        ...
        self._logger.register_key('Safety/shield_violation')
        self._logger.register_key('Safety/shield_adv_c')
    ...
    def _update(self) -> None:
        ...
        """
        data = self._buf.get()
        obs, act, logp, target_value_r, target_value_c, adv_r, adv_c, target_shield_violation, shield_adv_c = (
            data['obs'],
            data['act'],
            data['logp'],
            data['target_value_r'],
        ...
        dataloader = DataLoader(
            dataset=TensorDataset(obs, act, logp, target_value_r, target_value_c, adv_r, adv_c, target_shield_violation, shield_adv_c),
        ...
        for i in track(range(self._cfgs.algo_cfgs.update_iters), description='Updating...'):
            for (
                ...
                adv_r,
                adv_c,
                target_shield_violation,
                shield_adv_c,
            ) in dataloader:
                self._update_reward_critic(obs, target_value_r)
                if self._cfgs.algo_cfgs.use_cost:
                    self._update_cost_critic_q_and_v_critic(obs, act, target_value_c, adv_c, target_shield_violation, shield_adv_c)
                self._update_actor(obs, act, logp, adv_r, adv_c, shield_adv_c)
    ...
    def _update_cost_critic_q_and_v_critic(self, obs: torch.Tensor, act: torch.Tensor, target_value_c: torch.Tensor, target_value_adv_c: torch.Tensor, target_shield_violation: torch.Tensor, shield_adv_c: torch.Tensor) -> None:
        r"""Update value network under a double for loop.
        ...
        Args:
            ...
            target_shield_violation (torch.Tensor): The ``target_shield_violation`` sampled from buffer.
            shield_adv_c (torch.Tensor): The ``shield_adv_c`` sampled from buffer.
        """
        self._actor_critic.cost_q_critic_optimizer.zero_grad()
        self._actor_critic.cost_v_critic_optimizer.zero_grad()
        self._actor_critic.shield_violation_critic_optimizer.zero_grad()

        cost_v_grad = self._actor_critic.cost_v_critic(obs)[0]
        loss_v = nn.functional.mse_loss(cost_v_grad, target_value_c)
        loss_shield_violation = nn.functional.mse_loss(self._actor_critic.shield_violation_critic(obs)[0], target_shield_violation)
        ...
        if self._cfgs.algo_cfgs.use_critic_norm:
            ...
            for param in self._actor_critic.shield_violation_critic.parameters():
                loss_shield_violation += param.pow(2).sum() * self._cfgs.algo_cfgs.critic_norm_coef

        loss_v.backward()
        loss_adv.backward()
        loss_shield_violation.backward()
        if self._cfgs.algo_cfgs.use_max_grad_norm:
            clip_grad_norm_(
            ...
            clip_grad_norm_(
                self._actor_critic.shield_violation_critic.parameters(),
                self._cfgs.algo_cfgs.max_grad_norm,
            )
        distributed.avg_grads(self._actor_critic.cost_q_critic)
        distributed.avg_grads(self._actor_critic.cost_v_critic)
        distributed.avg_grads(self._actor_critic.shield_violation_critic)
        self._actor_critic.cost_q_critic_optimizer.step()
        self._actor_critic.cost_v_critic_optimizer.step()
        self._actor_critic.shield_violation_critic_optimizer.step()

        self._logger.store({'Loss/Loss_cost_critic': loss_v.mean().item() + loss_adv.mean().item() + loss_shield_violation.mean().item()})
    ...
    def _compute_adv_surrogate(  # pylint: disable=unused-argument
        ...
        Args:
            adv_r (torch.Tensor): The ``reward_advantage`` sampled from buffer.
            safety_bonus (torch.Tensor): The ``safety_bonus`` sampled from buffer.
            shield_adv_c (torch.Tensor): The ``shield_adv_c`` sampled from buffer.
        Returns:
    ...
    def _loss_pi(
        ...
        ratio = torch.exp(logp_ - logp).clamp(min=0.0, max=10.0)
        loss = -(ratio * adv).mean()
