...
class BaseDictBuffer(ABC):
    ...
    Args:
        ...
        act_space (OmnisafeSpace): The action space (expected to be Box).
        size (int): The size of the buffer.
    ...
    Raises:
        NotImplementedError: If the observation space is not a Dict or action space is not a Box.
    """
    ...
    def __init__(
        ...
        if not isinstance(act_space, gym.spaces.Box):
            raise NotImplementedError("Action space must be a Box")
        act_buf = torch.zeros((size, *act_space.shape), dtype=torch.float32, device=device)
