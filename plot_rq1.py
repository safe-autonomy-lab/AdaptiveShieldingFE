...
def create_mixed_environment_plot(
    ...
    # ShieldACP -> Shield, SafetyObjOnly -> SRO
    ours = ["FETRPOShieldACP", "FEnoACP_TRPOSafetyObjOnly"]
    
    # Define environment-specific algorithms
    ENV_SPECIFIC_ALGORITHMS = {
        # Point Robot
        "Point-Goal": baselines + ours + ["FETRPOShield_s5_b1_t0.275"], 
        "Point-Button": baselines + ours + ["FETRPOShield_s5_b1_t0.275"],
        "Point-Push": baselines + ours + ["FETRPOShield_s10_b0.1_t0.275"],
        "Point-Circle": baselines + ours + ["FETRPOShield_s100_b1_t0.275"],
        # Car Robot
        "Car-Goal": baselines + ours + ["FETRPOShield_s5_b1_t0.275"],
        "Car-Button": baselines + ours + ["FETRPOShield_s5_b1_t0.275"],
        "Car-Push": baselines + ours + ["FETRPOShield_s10_b0.1_t0.275"],
        "Car-Circle": baselines + ours + ["FETRPOShield_s100_b1_t0.275"],        
    }
    ...
    "OracleUSL": "USL",
    # ours
    "FETRPOShieldACP": "Shield",
    "FEPPOShieldACP": "Shield",
    "FEnoACP_TRPOSafetyObjOnly": "SRO",
    "FETRPOShield_s{sampling_nbr}_b{safety_bonus}_t0.275": "SRO + Shield",
    }

    # Group labels for legend
    BASELINE_GROUP = ["TRPO-Lag", "PPO-Lag", "CPO", "USL"]
    ...
    for env_idx, env_name in enumerate(env_order):
        ...
        for metric_idx, metric in enumerate(metrics):
            ...
            for alg in current_algs:
                ...
                mean_np = mean.to_numpy() if hasattr(mean, 'to_numpy') else np.array(mean)
                std_np = std.to_numpy() if hasattr(std, 'to_numpy') else np.array(std)
                
                ax.plot(x, mean_np, label=label_alg, color=COLORS[label_alg], linewidth=2.5)
                ax.fill_between(x, mean_np - std_np, mean_np + std_np, alpha=0.15, color=COLORS[label_alg])
            ...
            # elif metric_idx == 1:
            #     ax.set_title("Cost", fontsize=FONT_SIZES["title"])
    ...
    # Create legend with two separate boxes - one for baselines and one for our methods
    baseline_handles = []
    baseline_labels = []
    ours_handles = []
    ours_labels = []
    ...
    baseline_legend = fig.legend(
        ...
        baseline_labels,
        loc='lower center',
        bbox_to_anchor=(0.27, 1.0),  # Place it in the bottom center-left
        ncol=len(baseline_labels),  # One column per algorithm
        frameon=True,
        title="Baselines",
    ...
    # Create our methods legend - position at the bottom right
    ours_legend = fig.legend(
        ours_handles,
        ours_labels,
        loc='lower center', 
        bbox_to_anchor=(0.75, 1.0),  # Place it in the bottom center-right
        ncol=3,  # Arrange in two rows
        frameon=True,
        title="Our Methods",
        title_fontsize=FONT_SIZES["legend"],
        fontsize=FONT_SIZES["legend"],
    ...
    # Make sure both legends are added to the figure
    fig.add_artist(baseline_legend)
    fig.add_artist(ours_legend)

    plt.tight_layout()
    # Add more room at the bottom for the separate legends
    plt.subplots_adjust(bottom=0.18, top=0.94)
    return fig

def create_comparison_plots(robot_type: str, base_path: str, output_dir: str = "plot_main_figures") -> None:
    ...
    mixed_output = output_path / f"rq1_{robot_type}.png"
