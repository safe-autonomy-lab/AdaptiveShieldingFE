...
class FunctionEncoder(torch.nn.Module):
    ...
    def train_model(self,
        ...
        for epoch in bar:
            ...
            for batch_idx in range(n_batches):
                ...
                dataset.n_functions = min(current_batch_size, total_functions - batch_idx * current_batch_size)
    ...
    def save_initial_coefficients(self, coefficients:torch.tensor):
        """ Save the initial coefficients of the function encoder to a file. """
        self.coefficients = coefficients
    
    def load(self, path: str, device='cuda' if torch.cuda.is_available() else 'cpu'):
        """ Load the function encoder from a file. """
        state_dict = torch.load(path, map_location=device)
        
        # Handle case where saved model doesn't have average_function weights
        # but current model expects them (architecture mismatch)
        missing_keys, unexpected_keys = self.load_state_dict(state_dict, strict=False)
        
        if missing_keys:
            avg_function_missing = [k for k in missing_keys if k.startswith('average_function.')]
            other_missing = [k for k in missing_keys if not k.startswith('average_function.')]
            
            if avg_function_missing and not other_missing:
                print(f"WARNING: Saved model is missing average_function weights ({len(avg_function_missing)} keys).")
                print("This usually means the model was saved without use_residuals_method=True.")
                print("The average_function will be randomly initialized.")
                # This is OK - the average function will just be randomly initialized
            elif other_missing:
                raise RuntimeError(f"Missing critical model weights: {other_missing[:5]}...")
