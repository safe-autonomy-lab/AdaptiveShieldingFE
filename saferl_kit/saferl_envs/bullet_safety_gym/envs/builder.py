...
from saferl_kit.saferl_envs.bullet_safety_gym.envs.obstacles import create_obstacles
from saferl_kit.saferl_envs.bullet_safety_gym.envs import bases, worlds, tasks, agents
import os
import pkgutil
...
class EnvironmentBuilder(gym.Env):
    ...
    def step(
        ...
        if self.iteration > 1999:
            truncated = True
        else:
            truncated = False
        next_obs = self.get_observation()
        info['hidden_parameters'] = [self.agent.max_force, self.agent.max_velocity, self.agent.mass]
        info['agent_pos'] = self.agent.get_position()
        info['agent_vel'] = self.agent.get_velocity
        info['dist2bound'] =  np.abs(self.agent.get_position()[1]) - self.task.y_lim
