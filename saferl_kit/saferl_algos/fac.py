...
from saferl_kit.saferl_utils import C_Critic,Critic,Actor,MultiplerNet
...
def eval_policy(policy, eval_env, seed, flag):
    """
    Evaluate policy over multiple episodes.
    
    Args:
        policy: Policy to evaluate
        eval_env: Environment to evaluate in
        seed: Random seed
        flag: Flag to use for cost calculation
        
    Returns:
        tuple: Average episode reward and cost
    """
    def process_state(state):
        if isinstance(state, dict):
            return np.concatenate([v.flatten() if isinstance(v, np.ndarray) else np.array([v]) 
                                 for v in state.values()])
        elif isinstance(state, (list, tuple)):
            return np.concatenate([np.array(x).flatten() for x in state])
    ...
    for _ in range(eval_episodes):
        ...
        while not (terminated or truncated):
            # Process state before passing to select_action
            processed_state = process_state(state)
            action = policy.select_action(processed_state)
            
            state, reward, cost, terminated, truncated, info = eval_env.step(action)
            info['cost'] = cost
            avg_reward += reward
            avg_cost += 1. if info[flag] else 0.
